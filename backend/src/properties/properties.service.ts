...
export class PropertiesService {
    ...
    if (userId) {
      ...
      if (user && user.planId) {
        ...
        if (
          ...
          const currentCount = await this.propertiesRepository.count({
            where: { userId },
          });

          if (plan.propertyLimit > 0 && currentCount >= plan.propertyLimit) {
            throw new BadRequestException(
              `Limite de imóveis do plano ${plan.name} atingido (${plan.propertyLimit} imóveis). Faça um upgrade para adicionar mais.`,
            );
          }
        }
      }
  ...
  async findByLocation(
    ...
    const query = this.propertiesRepository
      ...
      .andWhere('property.location LIKE :neighborhood', {
